...
void loop() {
  ...
  if (checkIntersection()){
        ...
        blink(INTERSECT_BLINK_RATE);
  } 

  else if ((reading[3] == 1 || reading[4] == 1 || reading[5] == 1) && right_reading == 1){
     do {
       updateIr();
       iReadArray();
       motor->go((iMotorSpeed - REDUCE), (iMotorSpeed - REDUCE), FORWARD);
     } while (reading[3] == 1 || reading[4] == 1);
     
     motor->go(0, 0, NOWHERE);
     blink(BLINK_RATE);
     do {
       iReadArray();
       motor->go((iMotorSpeed - REDUCE) , (iMotorSpeed - REDUCE), CLOCKWISE);
     } while (reading[7] == 0);
